...
if ( ! class_exists( 'Clothes_Rental_Services_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 ...
	 * @since Clothes Rental Services 1.0
	 */
	class Clothes_Rental_Services_Walker_Page extends Walker_Page {

		/**
		 * Outputs the beginning of the current element in the tree.
		 *
		 * @since Clothes Rental Services 1.0
		 * @since Clothes Rental Services 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'clothes-rental-services' ), $page->ID );
			}
